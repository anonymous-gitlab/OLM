...
class MultimmodalCATransformer(Module):
    def __init__(self,
                 in_chans: int = 1024,
                 d_model: int = 512,
                 nhead: int = 8,
        ...
        super(MultimmodalCATransformer, self).__init__()
        ...
        MUL_visual_encoder_layer = TransformerDecoderLayer(d_model=self.embed_dim, nhead=nhead)
        MUL_audio_encoder_layer = TransformerDecoderLayer(d_model=self.embed_dim, nhead=nhead)
    ...
    def forward(self, src_v, src_key_padding_mask_v, src_a, src_key_padding_mask_a, src_t, src_key_padding_mask_t):        
        ...
        for mod_v, mod_a in zip(self.multimodal_vision_layers, self.multimodal_aduio_layers):
            output_v = mod_v(output_v, output_a, tgt_key_padding_mask = src_key_padding_mask_v, memory_key_padding_mask = src_key_padding_mask_a)
            output_a = mod_a(output_a, output_v, tgt_key_padding_mask = src_key_padding_mask_a, memory_key_padding_mask = src_key_padding_mask_v)
            output_p = torch.cat([output_v,output_a], dim=0)
