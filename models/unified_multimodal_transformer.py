...
class UnifiedMultimodalTransformer(Module):
    def __init__(self,
                 in_chans: int = 1024,
                 d_model: int = 512,
                 nhead: int = 8,
        ...
        super(UnifiedMultimodalTransformer, self).__init__()
        ...
        TOKEN_collection_vision_layer = TransformerDecoderLayer(d_model=self.embed_dim, nhead=nhead)
        TOKEN_collection_audio_layer = TransformerDecoderLayer(d_model=self.embed_dim, nhead=nhead)
        TOKEN_propagation_vision_layer = TransformerDecoderLayer(d_model=self.embed_dim, nhead=nhead)
        TOKEN_propagation_audio_layer = TransformerDecoderLayer(d_model=self.embed_dim, nhead=nhead)
    ...
    def forward(self, src_v, src_key_padding_mask_v, src_a, src_key_padding_mask_a, src_t, src_key_padding_mask_t):        
        ...
        for col_v, col_a, pro_v, pro_a in zip(self.token_collection_vision_layers,
            ...
            neck_vision = col_v(output_p, output_v, memory_key_padding_mask=src_key_padding_mask_v) # neck_vision.shape (neck_size,batch_size, embedding_dim)
            neck_audio = col_a(output_p, output_a, memory_key_padding_mask=src_key_padding_mask_a)  # neck_audio.shape (neck_size, batch_size, embedding_dim)
            output_p = output_p + (neck_vision + neck_audio) / 2.0
            output_v = pro_v(output_v, output_p,tgt_key_padding_mask=src_key_padding_mask_v)
            output_a = pro_a(output_a, output_p,tgt_key_padding_mask=src_key_padding_mask_a)
