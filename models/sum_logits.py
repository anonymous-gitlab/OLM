...
class SummationLogits(nn.Module):
    def __init__(self, audio_dim, text_dim, video_dim, output_dim1, output_dim2=1, layers='256,128', dropout=0.3):
        ...
        hiddendim = layers_list[-1] * 1
        # hiddendim = layers_list[-1] * 2
        # hiddendim = layers_list[-1] * 3
        
        # self.sum_fused_mlp = self.MLP(hiddendim, layers, dropout)
        self.sum_fused_mlp = self.MLP(hiddendim, layers, dropout)
    ...
    def forward(self, audio_feat, text_feat, video_feat, emotions, train=False):
        ...
        multi_hidden1 = torch.sum(torch.stack([audio_hidden, video_hidden, text_hidden], dim=0), dim=0) # [32, 128]
        fused_feat = self.sum_fused_mlp(multi_hidden1) # [32, 128]        
