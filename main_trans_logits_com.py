...
from engines.engine_trans_logits_com import train_or_eval_model
...
def main(args, configs):
    ...
    for fold in range(args.num_folder):
        ...
        for epoch in range(args.epochs):
            ...
            train_results = train_or_eval_model(args, model, cls_loss, train_loader, eval_loader, optimizer_network=optimizer_network,
                                                optimizer_logits=optimizer_logits, weights=weights, iteration=iteration, train=True)
            eval_results = train_or_eval_model(args, model, cls_loss, train_loader, eval_loader, optimizer_network=None, optimizer_logits=None, train=False)
            iteration += 1
...
if __name__ == '__main__':
    ...
    ## params for input
    parser.add_argument('--config', type=str, default='./configs/train_val_test1.yaml', help='config files')
