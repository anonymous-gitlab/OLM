...
class CramedDataset(Dataset):
    def __init__(self, args, mode='train'):
        ...
        self.data_root = '/home/xxx/cv/logit_modulation/data/CREMAD/'
        class_dict = {'NEU':0, 'HAP':1, 'SAD':2, 'FEA':3, 'DIS':4, 'ANG':5}
       
        if args.dataset  == 'CREMAD':
            self.audio_feature_path = '/home/xxx/cv/logit_modulation/data/CREMA-D/AudioWAV/'
            self.visual_feature_path = '/home/xxx/cv/logit_modulation/data/CREMA-D/'
    ...
    def __getitem__(self, idx):
        # audio
        samples, rate = librosa.load(self.audio[idx], sr=22050)    # ??? 为什么是22050而不是16000
        resamples = np.tile(samples, 3)[:22050*3]
        # samples, rate = librosa.load(self.audio[idx], sr=16000)  # ??? 为什么是22050而不是16000
        # resamples = np.tile(samples, 3)[:16000*3]
