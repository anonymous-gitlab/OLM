...
class MultimodalBottleneckTransformer(Module):
    def __init__(self,
                 in_chans: int = 1024,
                 d_model: int = 512,
                 nhead: int = 8,
                 num_encoder_layers: int = 3,
                 start_fusion_layer: int = 0,
                 pivot_num: int = 12,
        ...
        MUL_visual_encoder_layer = TransformerEncoderLayer(d_model=self.embed_dim, nhead=nhead)
        MUL_audio_encoder_layer = TransformerEncoderLayer(d_model=self.embed_dim, nhead=nhead)
    ...
    def forward(self, src_v, src_key_padding_mask_v, src_a, src_key_padding_mask_a, src_t, src_key_padding_mask_t):        
        ...
        src_key_padding_mask_v = torch.cat((src_key_padding_mask_v, self.false_column.expand(batch_size, shared_pivot.shape[0])), dim=1)
        src_key_padding_mask_a = torch.cat((src_key_padding_mask_a, self.false_column.expand(batch_size, shared_pivot.shape[0])), dim=1)
        
        output_v = src_v
        output_a = src_a
        output_p = shared_pivot
        
        vision_seq_len = output_v.shape[0]
        audio_seq_len = output_a.shape[0]
        ...
        # multimodal encoder
        for mod_v, mod_a, in zip(self.multimodal_vision_layers, self.multimodal_aduio_layers):
            vison_neck = torch.cat((output_v,output_p), dim=0)
            audio_neck = torch.cat((output_a,output_p), dim=0)
            output_v = mod_v(vison_neck, src_key_padding_mask=src_key_padding_mask_v)[:vision_seq_len,:,:]
            z_fsn_v = mod_v(vison_neck, src_key_padding_mask=src_key_padding_mask_v)[vision_seq_len:,:,:]
            output_a = mod_a(audio_neck, src_key_padding_mask=src_key_padding_mask_a)[:audio_seq_len,:,:]
            z_fsn_a  = mod_a(audio_neck, src_key_padding_mask=src_key_padding_mask_a)[audio_seq_len:,:,:]
            output_p = (z_fsn_v + z_fsn_a) / 2.0
        ...
        # Step 4: Take Average.
        src_key_padding_mask_v = src_key_padding_mask_v[:,:vision_seq_len]
        src_key_padding_mask_a = src_key_padding_mask_a[:,:audio_seq_len]
